...
//     cv::imwrite("means_dump.tiff", chipimgproc::viewable(md, 0.02, 0.02));
// }
auto get_banff_gridder(const std::string& patname ) {
    chipimgproc::marker::TxtToImg txt_to_img;
    std::ifstream marker_in(
        ( nucleona::test::data_dir() / "banff_rc" / patname).string()
    );
    chipimgproc::comb::SingleGeneral<> gridder;
...
//     using FLOAT = float;
//     auto gridder = get_banff_gridder("pat_CY3.tsv");
//     auto test_img_paths = {
...
TEST(single_image_general_gridding, banff_grid_hard_test) {
    using FLOAT = float;
    auto gridder = get_banff_gridder("pat_CY5.tsv");
    auto test_img_paths = {
